...
export default function PostCard({ post }){
  const { toggleLike, addComment, removePost, currentUser } = useApp()
  ...
  // Klik header: buka modal detail di desktop, jangan override klik link ke profil
  ...
  // Gunakan avatar author jika ada, fallback ke avatar profil pengguna jika post milik pengguna saat ini
  const isMeAuthor = post?.author?.username && currentUser?.username === post.author.username
  const avatarUrl = post?.author?.avatar || (isMeAuthor && currentUser?.avatar) || ''
  const avatarStyle = avatarUrl ? { backgroundImage: `url(${avatarUrl})` } : {}

  return (
    <article className="post">
      <header className="post-header clickable" onClick={onHeaderClick}>
        <div className="avatar" aria-hidden="true" style={avatarStyle} />
        <div className="meta">
        ...
        <button className="header-open" onClick={(e)=>{ e.stopPropagation(); setShowModal(true); }} aria-label="Lihat detail">
      ...
      {showModal && (
        isMobile ? (
          ...
          <div className="image-modal" onClick={()=>setShowModal(false)} role="dialog" aria-modal="true">
            <button className="close" onClick={()=>setShowModal(false)} aria-label="Tutup">×</button>
            <div className="image-modal-content two-col" onClick={(e)=>e.stopPropagation()}>
              <div className="image-panel">
                {imgs.length > 1 && <button className="nav prev" onClick={prev}>‹</button>}
                <img src={imgs[index]} alt="detail" />
                {imgs.length > 1 && <button className="nav next" onClick={next}>›</button>}
                {imgs.length > 1 && (
                  <div className="dots">
                    {imgs.map((_, i)=> (
              ...
              <aside className="details-panel">
                ...
                <div className="actions">
                  ...
                  <button className="comment" onClick={()=> { const isDesktop = window.matchMedia('(min-width: 1025px)').matches; if (isDesktop) { setShowModal(true); setShowComment(true); } else { setShowComment(true); } setTimeout(()=> inputRef.current?.focus(), 0); }} aria-label="Komentar">
                    <span className="icon" aria-hidden="true">
                      <svg viewBox="0 0 24 24" width="18" height="18">
